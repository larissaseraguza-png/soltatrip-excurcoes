...
export const Route = createFileRoute("/invite/excursionista/$id")({
  ...
  component: ExcursionistaInvitePage,
});
...
function ExcursionistaInvitePage() {
  ...
  const { data: vitrine, isLoading: loadingV } = useQuery({
    queryKey: ["vitrine", orgId],
    queryFn: async () => {
      const { data, error } = await supabase
        .rpc("get_excursionista_vitrine", { p_org_id: orgId })
        .maybeSingle();
  ...
  const { data: excursoes = [], isLoading: loadingE } = useQuery({
    queryKey: ["vitrine-excursoes", orgId],
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_excursionista_excursoes_publicas", {
        p_org_id: orgId,
      });
      if (error) throw error;
  ...
  // Auto-link se já está logado como passageiro
  useEffect(() => {
    if (authLoading || roleLoading || linking) return;
  ...
  function handleEntrar() {
    rememberExcursionistaInvite(orgId);
    navigate({ to: "/auth" });
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background px-4">
        <div className="glass rounded-3xl p-8 max-w-md text-center">
          ...
          <h1 className="mt-3 font-display text-xl font-bold">Link inválido</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Esse convite não existe ou foi removido. Peça um novo link ao seu organizador.
          </p>
          ...
          <div className="relative">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                <div className="text-[10px] font-bold tracking-[0.18em] text-neon-pink">CONVITE</div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Ao entrar você terá acesso às excursões deste organizador. Crie sua conta ou faça
                    login para reservar.
                  </p>
                </div>
              </div>
